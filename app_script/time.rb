...
#parser = CronParser.new("*/15 * * * *")
#now = "2011-08-15 02:02"
#puts "FOO: " + parser.next(now)


def parse_element(subel)
 if subel =~ /^\*/
        step = subel.length > 1 ? subel[2..-1].to_i : 1
        puts "Every: " + step.to_s
      else
        if SUBELEMENT_REGEX === subel
          if $5 # with range
            stepped_range($1.to_i..($3.to_i + 1), $5.to_i)
...
puts "Next to run: " + future.strftime("%Y-%m-%d %H:%M:%S")